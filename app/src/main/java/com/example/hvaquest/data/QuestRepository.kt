...
class QuestRepository {
    private val hvaQuest: List<Question> = arrayListOf(
            Question(
    ...
    fun getHvaQuest(index: Int): Question? {
        return if (index <= hvaQuest.size && index >= 0){
            hvaQuest[index - 1]
        } else {
            null
        }
    ...
    fun getQuestSize(): Int = hvaQuest.size
}
